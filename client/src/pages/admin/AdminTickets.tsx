...
import { Ticket, Loader2, Save, X, ExternalLink, Image as ImageIcon } from 'lucide-react'
  ...
  const columns: ColumnDef<TicketData>[] = [
      ...
      cell: ({ row }) => (
        <div className="flex items-center space-x-3">
          <div className="w-10 h-10 rounded overflow-hidden bg-secondary flex items-center justify-center">
            {row.original.img ? <img src={row.original.img} alt="" className="w-full h-full object-cover" /> : <ImageIcon className="w-4 h-4" />}
          </div>
    ...
    { accessorKey: 'client.username', header: 'Client' },
    {
      ...
      cell: ({ row }) => {
        ...
        const color = s === 'pending' ? 'bg-yellow-100 text-yellow-700' :
                      s === 'negotiating' ? 'bg-blue-100 text-blue-700' :
                      s === 'accepted' ? 'bg-green-100 text-green-700' :
                      s === 'rejected' || s === 'cancelled' ? 'bg-red-100 text-red-700' :
                      'bg-gray-100 text-gray-700'
        return <span className={`px-2 py-1 rounded-full text-xs font-medium uppercase ${color}`}>{s}</span>
      }
    },
    {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-4">
              ...
              <div className="flex space-x-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Client's Budget: <span className="font-medium text-foreground">{selectedTicket.expected_price ? `฿${selectedTicket.expected_price}` : 'Not specified'}</span></p>
            ...
            <form onSubmit={handleUpdate} className="space-y-4 bg-secondary/50 p-4 rounded-lg">
              ...
              <div>
                ...
                <select value={form.status} onChange={e => setForm({...form, status: e.target.value})} className="w-full p-2 border border-border rounded bg-background">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="number" value={form.proposed_price_jpy} onChange={e => setForm({...form, proposed_price_jpy: e.target.value})} placeholder="e.g. 1500" className="w-full p-2 border border-border rounded bg-background" />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1">Price (THB)</label>
                  <input type="number" value={form.proposed_price_thb} onChange={e => setForm({...form, proposed_price_thb: e.target.value})} placeholder="e.g. 500" className="w-full p-2 border border-border rounded bg-background" />
                </div>
              </div>

              <div>
                <label className="block text-sm font-medium mb-1">Assign to Trip (Optional)</label>
                <select value={form.trip_id} onChange={e => setForm({...form, trip_id: e.target.value})} className="w-full p-2 border border-border rounded bg-background">
                  <option value="">-- No Trip Assigned --</option>
                  {trips?.map((t: any) => (
                    <option key={t.id} value={t.id}>Trip #{t.id} ({new Date(t.ship_date).toLocaleDateString()}) - {t.type.toUpperCase()}</option>
                  ))}
                </select>
              </div>

              <div className="flex justify-end pt-2">
                <button type="submit" disabled={updateTicketMutation.isPending} className="bg-primary text-primary-foreground px-4 py-2 rounded flex items-center">
