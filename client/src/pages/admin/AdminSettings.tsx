      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
        <div className="md:col-span-1 space-y-1">
          <button 
            ...
            className={`w-full text-left px-4 py-2 font-medium rounded-lg transition-colors ${activeTab === 'profile' ? 'bg-primary/10 text-primary' : 'text-muted-foreground hover:bg-secondary'}`}
          >
            Profile
          </button>
          <button 
            onClick={() => setActiveTab('security')}
            className={`w-full text-left px-4 py-2 font-medium rounded-lg transition-colors ${activeTab === 'security' ? 'bg-primary/10 text-primary' : 'text-muted-foreground hover:bg-secondary'}`}
          >
            Security
          </button>
        </div>
        ...
        <div className="md:col-span-3 space-y-6">
          {activeTab === 'profile' && (
            <form onSubmit={handleProfileUpdate} className="bg-card border border-border p-6 rounded-xl shadow-sm space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input 
                    ...
                    disabled 
                    value={user?.email || ''} 
                    className="w-full p-2 border border-border rounded bg-secondary text-muted-foreground cursor-not-allowed" 
                  />
                ...
                <div>
                  ...
                  <input 
                    ...
                    className="w-full p-2 border border-border rounded bg-secondary text-primary font-bold cursor-not-allowed" 
                  />
                </div>
              </div>
              
              <div className="pt-4 flex justify-end">
                <button type="submit" disabled={isUpdatingProfile} className="bg-primary text-primary-foreground px-6 py-2 rounded-lg hover:bg-primary/90 transition-colors flex items-center">
          ...
          {activeTab === 'security' && (
            <form onSubmit={handlePasswordUpdate} className="bg-card border border-border p-6 rounded-xl shadow-sm space-y-6">
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium mb-1 text-foreground">Current Password</label>
                  <input 
                    type="password" 
                    required
